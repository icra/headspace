...
#
# INPUT: 
#       You can either input a vector of 9 values for solvinf a single sample or a data frame of 9
...
Rheadspace <-  function(...){
  ...
  if (is.data.frame(arguments[[1]])) {
    ...
    if (dim(input.table)[2]!=9){
      stop("You should input a data frame with 9 columns. See the readme file or comments in the function", call.=FALSE)
    }else{
    ...
    stop("You should input either a data frame or a vector of 9 values. See the readme file or comments in the function", call.=FALSE)
  }
